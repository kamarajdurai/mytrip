...
async function generatePlan() {
  ...
  try {
    ...
    // Map the forecast data to a single entry per day
    ...
    days.forEach((d, i) => {
      ...
      if (f) {
        const desc = f.weather[0].description,
          temp = f.main.temp,
          wt = f.weather[0].main.toLowerCase();
        ...
        // Animate weather in sequence
        setTimeout(() => setWeatherAnimation(wt), i * 400);
      } else weatherInfo += `${d.toDateString()}: No data\n`;
    });

    // Wait for weather animations to start before calling Gemini API
    setTimeout(async () => {
